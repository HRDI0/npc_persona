...
| 모델 위치 | `./models/google-gemma-4-E4B-it` | 운영 모델 캐시 또는 기존 vLLM 설정 |
...
| 검증 항목 | 결과 |
| --- | --- |
| 모델 다운로드 | `models/google-gemma-4-E4B-it/model.safetensors` 존재 확인 |
...
| vLLM Docker image/API | `vllm/vllm-openai:latest` pull 완료, `/v1/models`에서 `google/gemma-4-E4B-it`, `max_model_len=2048` 확인 |
| vLLM chat completions | `POST /v1/chat/completions` → `CHAT_MODEL=google/gemma-4-E4B-it` 응답 확인 |
...
| 7 | 모델 선택 | VRAM 제약 때문에 `google/gemma-4-E4B-it`를 로컬 프로젝트 모델로 사용한다 | model/Compose 설정 표 |
...
모델은 VRAM 제약을 고려해 `google/gemma-4-E4B-it`로 맞췄다. 모델 파일은 전역 캐시가 아니라 프로젝트 내부 `models/google-gemma-4-E4B-it`에 내려받고, vLLM 컨테이너에는 `/models/gemma-4-E4B-it`로 read-only mount한다. vLLM은 `max_model_len=2048`, `gpu_memory_utilization=0.9`로 검증했다.
...
두 번째로 vLLM이 실제로 `google/gemma-4-E4B-it`를 서빙하고 있음을 확인한다.
...
| 모델 파일은 어디에 있는가? | 프로젝트 내부 `models/google-gemma-4-E4B-it`에 있으며 git과 Docker build context에서 제외된다. |
...
현재 MVP는 `rsc/data`의 세계관 원천을 Neo4j 그래프로 변환하고, Streamlit이 플레이어 상태에 맞는 `KnowledgeChunk`만 골라 `google/gemma-4-E4B-it` vLLM에 전달함으로써 NPC별 지식 범위와 퀘스트 공개 단계를 제어하는 구조다.
