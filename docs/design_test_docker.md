...
최신 실제 증거 캡처는 설계 검증 포트가 아니라 main Compose 포트에서 수행했다. Streamlit은 `127.0.0.1:8501`, vLLM은 `127.0.0.1:8000/v1/models`에서 확인했고, served model은 `google/gemma-4-E4B-it`, `max_model_len=4096`, `VLLM_GPU_MEMORY_UTILIZATION=0.8`이었다. `.env.design-test.example`은 여전히 `0.9`를 기본으로 둘 수 있으므로 같은 조건을 재현하려면 실행 환경에서 `VLLM_GPU_MEMORY_UTILIZATION=0.8`로 override한다.
