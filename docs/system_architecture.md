...
flowchart TD
    ...
    Log[JSONL interaction log append]
    DebugRender[chat-submit 블록 밖에서 debug expander 렌더]
    Error[예외 발생 시 오류 메시지 저장/표시]

    Start --> Config --> Page --> InitState --> Sidebar --> History --> Input
    Input --> AppendUser --> Profile --> Chunks --> Prompt --> SaveDebug --> Stream --> AppendAssistant --> Log
    Log --> DebugRender
    History --> DebugRender
    Profile -.예외.-> Error
    Chunks -.예외.-> Error
...
| `last_debug` | `{"retrieved_chunks": [], "prompt": ""}` | 마지막 조회 chunk와 최종 프롬프트 디버그 데이터 |

상태 선택값은 `get_allowed_chunks()`의 파라미터로 들어가고, 최종 프롬프트의 `[현재 대화 조건]`에도 포함된다. `last_debug`는 채팅 제출 처리 안에서 갱신되지만, expander 렌더링은 `st.chat_input` 블록 밖에서 수행되어 사이드바 변경이나 재실행 뒤에도 마지막 디버그 내용을 볼 수 있다.
...
| Debug Retrieved Chunks | `st.session_state.last_debug`에 저장된 마지막 조회 chunk 확인 |
| Debug Prompt | `st.session_state.last_debug`에 저장된 최종 프롬프트 확인 |
