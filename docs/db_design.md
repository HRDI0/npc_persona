...
Streamlit 메인 앱은 `NPC` 프로필과 해당 NPC가 `KNOWS`로 연결된 `KnowledgeChunk`만 조회해 vLLM OpenAI-compatible `/v1/chat/completions`에 프롬프트로 전달한다. Streamlit admin page는 별도로 `ConceptStory` standalone 노드를 확인하고 `MERGE`할 수 있다. RDB, embedding index, canonical ID alias 테이블은 이후 확장 단계로 남긴다.
...
src/streamlit/pages/admin.py Concept Story Admin 입력 -> ConceptStory
```
...
`ConceptStory`는 원천 importer의 입력 파일에서 만들어지는 노드가 아니다. admin page에서 운영자가 입력한 concept text를 `concept_id` 기준으로 확인하고 병합 저장하는 보조 노드다.
...
| `ConceptStory` | `concept_id` | `category`, `title`, `text`, `quest_id`, `npc_id`, `updated_at_ms` |

Importer는 원천 데이터 label인 `NPC`, `Role`, `Location`, `Quest`, `Event`, `Clue`, `Truth`, `KnowledgeChunk` 키에 대해 Neo4j uniqueness constraint를 생성한다. `ConceptStory.concept_id`는 admin page의 `MATCH`와 `MERGE`에 쓰는 논리 키지만, 현재 importer가 constraint를 만들지는 않는다.
...
`ConceptStory`는 현재 관계를 만들지 않는 standalone 노드다. `quest_id`와 `npc_id` 속성은 admin 입력값으로 저장될 수 있지만, `Quest`나 `NPC`로 이어지는 relationship은 생성하지 않는다.

## ConceptStory admin 계약

Concept Story Admin은 두 가지 DB 작업만 수행한다.
...
이 기능은 기존 concept text 확인과 새 concept text 저장을 위한 admin 도구다. chat memory를 수정하지 않고, `memory_by_npc`나 `memory_summary_by_npc`에도 쓰지 않는다. 또한 `KnowledgeChunk` 검색 경로와 분리되어 있으므로 `get_allowed_chunks()`의 후보, 필터, 정렬, limit에 참여하지 않는다.
